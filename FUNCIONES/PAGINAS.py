        ...
        if df_solicitud_seleccionada.shape[0] == 0:
            ...
            if archivo_AA is not None and io_envio is True:
                ...
                # En caso contrario procesa los datos
                    ...
                    # Extrae un listado de las muestras procesadas
                    df_temp = datos_corregidos.loc[datos_corregidos['salinidad_ctd'].notnull(), ['id_externo']]
                    listado_muestras_procesadas = df_temp["id_externo"]
...
def entrada_datos_excel():
    ...
    if archivo_datos is not None and io_envio is True:
        ...
        # Cambia los nombres de las variables a minusculas
        df_datos_importacion.columns = df_datos_importacion.columns.str.lower()
        
        # Elimina posibles espacios en blanco en el nombre de las variables
        df_datos_importacion.columns = df_datos_importacion.columns.str.strip()
    ...
    with st.form("Formulario seleccion"):  
        ...
        if io_envio == True:
            ...
            for idato in range(df_modificado.shape[0]):
                ...
                st.text(df_modificado["lote"].iloc[idato])    
...
def consulta_estado():
    ...
    ### Encabezados y titulos 
    #st.set_page_config(page_title='CONSULTA DATOS', layout="wide",page_icon=logo_IEO_reducido) 
    st.title('Estado de las analíticas encargadas al servicio de nutrientes del C.O de A Coruña')
    
    # Recupera la tabla de los programas disponibles como un dataframe
    conn = init_connection()
    df_solicitudes = pandas.read_sql('SELECT * FROM servicio_nutrientes_entradas', conn)
    df_estados = pandas.read_sql('SELECT * FROM planificacion_analisis_nutrientes', conn)
    conn.close()
    ...
    for isolicitud in range(df_solicitudes.shape[0]):
        ...
        df_solicitudes["fecha_actualizacion"].iloc[isolicitud] = (df_proceso['dttime'].iloc[0]).date()
